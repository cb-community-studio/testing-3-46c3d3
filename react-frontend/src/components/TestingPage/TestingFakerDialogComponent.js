...
import testingFakerFactory from "./testingFakerFactory";
...
const TestingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = testingFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("testing").create(elem));
...
export default connect(null, mapDispatch)(TestingFakerDialogComponent);
